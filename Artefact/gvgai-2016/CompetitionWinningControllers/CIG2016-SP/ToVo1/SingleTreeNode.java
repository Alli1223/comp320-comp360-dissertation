...
public class SingleTreeNode
{
    ...
    public static final double par_bestAction_visitsTolerance = 0.0;    //the relative tolerance [0-1] where nodes are still considered as equal in visits, when choosing the most visited action after search ends (0 - no tolerance)

    public static final int par_numMemorizedNodes = 1;    //with how many nodes to expand the tree in each iteration (-1 = all)
    ...
    public static final double par_forgettingRate_search = 0;   // 1.0 - all knowledge from the previous search will be forgotten, 0.0 - all knowledge will be retained
    public static final double par_playoutNonUniform = 0;   // from 0.0 to 1.0, value of 0.0 equals to completely uniform action selection in playout
    ...
    public static final int par_rollout_depth_increase_interval = 5;   // how often to increase the rollout depth (in episodes)
    public static final double par_rollout_depth_increase_rate = par_rollout_depth_increase_interval;    // how much to increase rollout depth after each batch of episodes
            ...
            if(!isPlayout){ //tree policy
                ...
                if (node.notFullyExpanded()) {  // Select random unvisited children
                    ...
                    for (SingleTreeNode child : node.children) {
                        ...
                        // the UCB1 equation
                        double uctValue = childValue + UCB_Cp * Math.sqrt(Math.log(node.nVisits + 1) / (child.nVisits + epsilon));
    ...
    public int mostVisitedActionEnhanced() {
        ...
        // find highest visit count and define the lower bound considering the specified threshold
        for (int i=0; i<this.children.length; i++) {
            if(this.children[i] != null){

                //decay the values in the tree (safety check, although these nodes will very likely be up to date, unless the algorithm computed very little episodes)
                if(this.children[i].lastEvaluationSearch < (currentSearchID-1)){
                    ...
                    this.children[i].nVisits = this.children[i].nVisits * Math.pow(1.0-par_forgettingRate_search, power);
