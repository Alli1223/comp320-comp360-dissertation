...
\section{Research Questions}
	...
	%Another question this paper will aim to answer is; \textit{What games have too large of a search space for what techniques?}.
...
\section{Literature Review}
	...
	\subsection{Game Search Techniques} \label{ssec:GST}
		...
		\textbf{Horizon Effect}
			 is the problem where only a small portion of the search tree can be searched, i.e. the AI can search 4 moves ahead, but the 5th move could be a detrimental move, but the AI doesn't know that because of its "\textit{horizon}".

		\textbf{Evaluation Function}
			is used to estimate the value of a position or the current game state, i.e. is the agent winning or loosing the game.
		...
		\textbf{Expansion}
			...
			Typically only one child is added to the tree by many MCTS based agents after selection and before the simulation \cite{chaslot2008monte, schuster2015mcts}. 
	...
	\subsubsection{The GVGAI Framework \& Sample Controllers} \label{Framework}
		...
		\textbf{Random}
			...
			The random controller came 14th in the competition, which is quite surprising as it managed to beat quite a few of the other, more complicated controllers. \footnote{The reason for this may be due to more complicated controllers not choosing an action because it isn't able to search the game space, so making a random action is often better than no action.}
...
\section{Research Methodologies}	
	The data of the GVG-AI sample controllers as well as the controllers submitted for the competitions over the last few years will be gathered. 

	% What data will be collected:
	Examples of the type of metrics that will be gathered from this comparative study:
	\begin{itemize}
		\item Success rate in a particular game.
		\item Where the agent was when it lost or won the game
		\item How much of the search space was the method able to search
		\item The amount of moves that controller made in each type of game
	\end{itemize}

	%How will the data be collected
	The data will be collected within the GVG-AI framework ( as mentioned in section \ref{Framework}, which is programmed in Java.

	%What I will create
	There will be thousands of different simulations to run to gather the required data, so this paper will propose an automated test software which will be able to run tests the majority of controllers and games. This will decrease the chance of human error interfering with the results.
	
	

	%Justification of research
	This software will provide an in depth insight into the challenges faced by each tree search algorithm.
	From this data conclusions can be drawn out as to the limitations of different search techniques and potentially any areas in which they could be improved.
...
\subsection{Hypothesis}
	...
	\textbf{Hypothesis One:}
		There is a clear weakness in one of the search techniques when trying to complete a specified type of game.
	\textbf{Hypothesis Two:}
		There is a clear Strength in one of
