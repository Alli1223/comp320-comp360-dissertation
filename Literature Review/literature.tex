...
\title{Comparing game tree search techniques for general videogame AI (GVGAI)}
%
%
% author name
...
\begin{itemize}
    ...
    \item and more.
\end{itemize}
These sample agents provide useful insigths into how new agents can be created for the competition by applying common AI techniques.
The \textit{HUMAN} player and the \textit{REPLAYER} can be used for debugging the gmae and to help the programmer get a better understading of how the game can be played.


The framework uses a Video Game Descriotion Language (VGDL) to describe a wide variety of video games. The VGDL is based on a python version developed by Schaul (2014) called PyVGDL \cite{schuster2015mcts}. Furthermore in the GVG-AI Competition the AI agent does not have access to the whole games description, where as in GGP the agent was able to see the whole game description. This means that the agent has to analyze and simulate the game in order to figure out the rules and goal of the game.

To create an agent in 

The framework has an interaction set that consists of 
