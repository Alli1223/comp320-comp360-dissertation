...
\section{Literature Review}
	...
	\subsection{Game Search Techniques}
		...
		\subsubsection{Evolutionary Algorithms} \label{sssec:EA}
			...
			Perez et al. in \cite{perez2013rolling} compared MCTS with RHEA on the game Physical Traveling Salesman Problem (PTSP). The game is a modification of the popular optimization problem, the Traveling Salesman Problem \cite{citationNeeded} where the player must visit a series of way points in a 2d map and the agent must execute an action in no more than 40ms \cite{perez2013rolling} which is similar to the GVG-AI competition.  The results rom that paper show that RHEA is a promising competitor to MCTS.
			...
			The term Rolling Horizon comes from the fact  that the planning happens until a limited look-ahead in the game (The Horizon). 
			

			\cite{gaina2017analysis, gaina2017rolling}
			...
			The figure \ref{fig:RHEArollout} shows the steps of RHEA, 

			%Combination of RHEA AND MCTS
			Recent literature on AGI have been combining evolution and tree search in interesting ways in order to combine the benefits of both methods, such as Licas et al. \cite{lucas2014fast} applied an EA process to guide the simulation step of MCTS and improve the random default policy. Their results show a significant increase in performace in the game \textit{Space Invaders} and the Mountain Car Problem.
			

		\subsubsection{Rolling Horizon} \label{sssec:RH}
			A popular 
...
Research Methodologies go here...
